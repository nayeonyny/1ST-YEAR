...
typedef struct {
    void (*sort)(void);  // Function pointer for sorting
    void (*display)(void);  // Function pointer for displaying the leaderboard
    const char *name;  // Name of the leaderboard
} Leaderboard;
...
void sortDivisionLeaderboard();
void DivisionLeaderboard();
void sortOverallLeaderboard();
void showLeaderboard();

// Array of leaderboards
Leaderboard leaderboards[] = {
    {sortAdditionLeaderboard, AdditionLeaderboard, "Addition Leaderboard"},
    {sortSubtractionLeaderboard, SubtractionLeaderboard, "Subtraction Leaderboard"},
    {sortMultiplicationLeaderboard, MultiplicationLeaderboard, "Multiplication Leaderboard"},
    {sortDivisionLeaderboard, DivisionLeaderboard, "Division Leaderboard"},
    {sortOverallLeaderboard, showLeaderboard, "Overall Leaderboard"}
};

// Number of leaderboards
const int leaderboardCount = sizeof(leaderboards) / sizeof(leaderboards[0]);
...
int main() {
    ...
    int choice;
    char currentUsername[50];  // Store the current logged-in username
...
void saveUsers() {
    ...
    // Save username, password, score, gamesPlayed, and games in each category
    for (int i = 0; i < userCount; i++) {
        fprintf(ptr, "%s %s %d %d %d %d %d %d %f\n", users[i].username, users[i].password, users[i].score, users[i].gamesPlayed, users[i].additionGames, users[i].subtractionGames, users[i].multiplicationGames, users[i].divisionGames, users[i].ave);
    }
...
void loadUsers() {
    ...
    // Load user username, password, score, gamesPlayed, and games in each category
    userCount = 0;
    while (fscanf(ptr, "%s %s %d %d %d %d %d %d %f", users[userCount].username, users[userCount].password, &users[userCount].score, &users[userCount].gamesPlayed, &users[userCount].additionGames, &users[userCount].subtractionGames, &users[userCount].multiplicationGames, &users[userCount].divisionGames, &users[userCount].ave) == 9) {
        userCount++;
...
void updateLeaderboard(char username[], int score, int gameType) {
    for (int i = 0; i < userCount; i++) {
        if (strcmp(users[i].username, username) == 0) {
            users[i].score += score;  // Add new score
            users[i].gamesPlayed++;  // Increment total games played
            if (gameType == 1) {
                users[i].additionGames += score;  // Increment addition games by score
            } else if (gameType == 2) {
                users[i].subtractionGames += score;  // Increment subtraction games by score
            } else if (gameType == 3) {
                users[i].multiplicationGames += score;  // Increment multiplication games by score
            } else if (gameType == 4) {
                users[i].divisionGames += score;  // Increment division games by score
            }
            users[i].ave = (float)users[i].score / users[i].gamesPlayed;  // Calculate average score
...
void AdditionLeaderboard() {
    ...
    for (int i = 0; i < userCount; i++) {
        if (users[i].additionGames > 0) {
            printf("%d. %s: \t\t%d \t\t%d\n", i + 1, users[i].username, users[i].additionGames, users[i].score);
        } else {
...
void SubtractionLeaderboard() {
    ...
    for (int i = 0; i < userCount; i++) {
        if (users[i].subtractionGames > 0) {
            printf("%d. %s:\t\t%d \t\t%d\n", i + 1, users[i].username, users[i].subtractionGames, users[i].score);
        } else {
...
void sortMultiplicationLeaderboard() {
    for (int i = 0; i < userCount - 1; i++) {
        for (int j = i + 1; j < userCount; j++) {
            if (users[j].multiplicationGames > users[i].multiplicationGames) {
                User temp = users[i];
                users[i] = users[j];
                users[j] = temp;
            }
...
void MultiplicationLeaderboard() {
    ...
    for (int i = 0; i < userCount; i++) {
        if (users[i].multiplicationGames > 0) {
            printf("%d. %s: \t\t%d \t\t%d\n", i + 1, users[i].username, users[i].multiplicationGames, users[i].score);
        } else {
...
void DivisionLeaderboard() {
    ...
    for (int i = 0; i < userCount; i++) {
        if (users[i].divisionGames > 0) {
            printf("%d. %s: \t\t%d \t\t%d\n", i + 1, users[i].username, users[i].divisionGames, users[i].score);
        } else {
...
void leaderboardMenu() {
    ...
    while (1) {
        ...
        for (int i = 0; i < leaderboardCount; i++) {
            printf("%d. %s\n", i + 1, leaderboards[i].name);
        }
        printf("%d. Back to Main Menu\n", leaderboardCount + 1);
        printf("Choose an option: ");
        scanf("%d", &choice);

        if (choice >= 1 && choice <= leaderboardCount) {
            leaderboards[choice - 1].sort();
            leaderboards[choice - 1].display();
        } else if (choice == leaderboardCount + 1) {
